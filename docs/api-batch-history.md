...
单文件中的静态摘要与 LLM 均是可选分析层。摘要提取抛错、LLM 调用异常或 LLM 返回结构无效时，接口仍返回 200 和真实的 LightGBM + MLP 结论，同时令 `llmVerdict` / `llmConfidence` 为 `null`，并在 `llmReport` 中写明降级原因。摘要失败时 `attck` 为空；仅 LLM 失败但摘要成功时，确定性 ATT&CK 标签继续保留。
