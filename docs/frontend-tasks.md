# 前端待办清单

> 本文件只记录前端工作，不直接修改 `webapp/frontend/`。后端接口以 `docs/api-batch-history.md` 和 FastAPI `/docs` 为准。

## P0：结果真实性与接口同步

### 1. 禁止网络失败时静默返回 mock 检测

现状：`src/lib/api.ts::analyzeFile()` 在 `fetch` 抛错时直接调用 `mockAnalyze()`，用户看到的结果外观与真实结果相同，可能把“后端未启动”误认为真实检测。

建议：
- 默认网络失败直接抛出 `DetectionFailedError("无法连接检测服务")`。
- 如仍需纯前端联调，仅在 `VITE_ENABLE_MOCKS=true` 时启用 mock。
- mock 模式必须在全局和结果卡片显示固定的“演示数据”标记，不能只藏在日志里。

验收：关闭后端上传文件时，正式模式只显示连接失败，不产生 verdict、家族名或历史记录。

### 2. 同步后端类型

在 `src/lib/types.ts` 补齐：
- `DetectionResult.historyId: number | null`
- `DetectionResult.featureAttention: FeatureAttention[] | null`
- `FeatureAttention { group, label, weight }`
- `HistoryRecord.featureAttention: FeatureAttention[]`（批量检测和迁移前旧记录为空数组）
- `HealthStatus`（含 `inferenceConcurrency`、`detectionConcurrency`、`apiKeyRequired`）、`HistoryStats`
- 批量检测的 `BatchItem`、`BatchDetectionResult`
- 历史记录 ID 统一使用 `number`，不要在 API 层转成随机字符串。

验收：类型与 `docs/api-batch-history.md` 一致，`npm run build` 无类型错误。

### 3. 修复单次检测后的历史重复

现状：后端检测成功时已经写入 SQLite，`App.tsx` 又通过 `toHistoryEntry()` 生成随机 ID 并追加本地记录；刷新后会从后端重新加载，容易出现身份不一致或重复。

建议：
- 检测成功后使用响应中的 `historyId`，或直接重新请求 `GET /api/history`。`historyId=null` 表示本次结果未保存（例如 SQLite 暂时不可写），仍应展示检测结论，但要显示“结果未保存”，并隐藏报告链接。
- 删除 `Date.now() + Math.random()` 生成历史 ID 的逻辑。
- HistoryPage 文案改为“记录保存在后端，刷新后仍可查看”，不再写“仅保存在当前浏览器会话”。

验收：连续检测两次、刷新页面后始终只有两条记录，ID 与后端一致。

### 4. 增加后端就绪状态

- 应用启动时请求 `GET /api/health`。
- `ready=false` 或检测返回 503 时，在上传区显示明确状态和 `modelLoadError`，并禁用上传按钮。
- `familyModelLoaded=false`、`llmConfigured=false` 是可选能力降级，不应阻止核心检测；家族状态结合 `familyModelLoadError` 显示“家族分类未加载”或“家族分类运行中已停用”，LLM 状态可显示“LLM 说明不可用”。不要因家族运行时降级清空已经返回的恶意/良性主结论。
- 不要把 `/api/health` 的 `ok=true` 误解为模型已经就绪，应使用 `ready/modelsLoaded`。

## P1：把现有后端能力真正展示出来

### 5. 三方结果与 LLM 说明

当前结果卡只展示 LightGBM、MLP 和 LLM 文本，没有展示 `llmVerdict/llmConfidence`。

建议采用紧凑的三列对比：LightGBM 恶意概率、MLP 恶意概率、LLM 独立判定及置信度。明确标注：
- 最终 verdict 只由 LightGBM + MLP 平均得到。
- LLM 是基于有限静态线索的独立辅助意见，不参与最终概率。
- `llmVerdict=null` 时根据 `llmReport` 显示“未配置”“摘要失败”或“LLM 分析失败”，不要显示 0%，也不要清空或遮挡已经返回的 LightGBM + MLP 主结论。摘要失败时 ATT&CK 可能为空；仅 LLM 失败时仍正常展示后端返回的确定性 ATT&CK 标签。

### 6. 特征组融合权重图

读取 `featureAttention`，按权重降序显示 12 个特征组，建议默认展示 Top 5，可展开全部。

措辞必须使用“特征组融合权重”或“模型内部关注权重”，不能写成“致因”“决定因素”或“该特征导致恶意判定”。注意力权重不是因果归因。

### 7. 完善历史页

- 请求 `GET /api/history/stats`，在表格上方展示总数、恶意/良性、模型分歧、LLM 分歧。
- 历史行增加来源（单文件/批量）、模型一致性、家族置信度。
- 历史详情读取 `featureAttention`，沿用“特征组融合权重/模型内部关注权重”的非因果措辞；数组为空时隐藏该区块，不补零。HTML 报告已由后端直接包含该表格。
- 增加查看详情、打开 `/api/history/{id}/report`、下载 `/api/history/backup`、删除单条和清空历史。
- 删除操作需要确认；失败时保留现有列表并显示错误。
- 数据较多时使用 `limit/offset` 分页，不一次拉取全部；读取列表响应头 `X-Total-Count` 计算总页数，即使当前页为空也不要把总数误判为 0。

### 8. 批量检测页面或模式

接入 `POST /api/detect/batch`：
- 支持多选/拖放，前端提前校验最多 100 个、单文件 100MiB、全部原始文件合计 500MiB；超过任一限制时不要开始上传。
- 展示总数、恶意、良性、失败统计和逐文件错误；整个请求收到 413 时显示批量总量错误，不要误归因到某一个文件。
- 明确提示批量模式不调用 LLM、不生成 ATT&CK 和融合权重。
- 后端仍在全部文件完成后一次性返回；前端只能区分上传中和服务端分析中，不能伪造“已扫描 X/100”的逐文件进度。
- 支持按 verdict/失败状态筛选，并允许跳转对应 `historyId` 的报告。

### 9. 移动端导航

当前 `<nav>` 在 `md` 以下直接隐藏，移动端只能看到“开始检测”，无法进入指标和历史页。增加标准菜单按钮和抽屉/下拉菜单，保持键盘可操作、焦点可见，并在路由切换后自动关闭。

## P2：体验与质量

### 10. 加载和错误状态细化

区分：上传中、ML 推理中、LLM 分析中目前后端仍是一次响应，前端至少使用不会误导的统一“分析中”；处理 401、413、422、429、503 和网络错误的不同文案。不要把后端返回的技术异常堆栈样式文本做成成功提示。

### 11. 可访问性与布局验证

- 上传控件、删除按钮、移动菜单提供键盘操作和可见焦点。
- 状态变化使用 `aria-live`，错误信息与上传控件关联。
- 检查 375px、768px、1440px 三档，无文本溢出、按钮遮挡或横向滚动。
- 最终在普通本地终端运行 `npm run lint` 和 `npm run build`；Windows 受限沙箱出现 `spawn EPERM` 时，不能直接判断为代码错误。

## 建议实施顺序

1. P0-1～P0-4：先保证结果真实、类型和状态正确。
2. P1-5～P1-7：完善单文件结果与历史闭环。
3. P1-8～P1-9：批量工作流和移动端导航。
4. P2：统一收尾并做响应式、可访问性检查。
## 本轮接口增量

### 12. 指标页展示部署集成结果

`GET /api/metrics` 现在有第三行“LightGBM + MLP 集成（部署模型）”。指标页不要假定只有两个模型，按返回数组渲染，并将部署模型作为默认强调行。它代表线上最终 verdict 使用的算术平均概率。接口返回 404 或 503 时显示“正式指标暂不可用”，不要补零、沿用旧值或回退到前端 mock。

增加一个紧凑的“指标来源”入口，按需请求 `GET /api/metrics/provenance`，展示测试集样本数、判定阈值、集成规则、评估时间、模型 SHA-256 前 12 位和 Git commit。接口 404 时只隐藏入口；接口 503 表示来源清单不完整或自相矛盾，应显示“指标来源不可用”，不得渲染部分清单或继续强调旧指标。

### 13. 错误提示附带请求编号

API 层统一读取响应头 `X-Request-ID`。后端现在保证业务错误和未捕获的 500 都返回该头；遇到 413、422、500、503 时，在详细信息中显示“请求编号”，便于后端排查。纯网络错误没有 HTTP 响应，不能伪造请求编号。正常成功页面无需展示，也不要记录或展示上传文件二进制内容。

### 14. 展示部署模型与正式指标的一致性

`GET /api/health` 新增 `modelProvenanceVerified` 和 `modelProvenanceWarning`：
- `true`：当前加载的 LightGBM、MLP、scaler 与正式评估清单哈希完全一致，可正常展示指标。
- `false`：模型文件已变化但指标未刷新；指标页显示醒目的“当前模型与评估版本不一致”，不要继续强调旧分数。
- `null`：评估清单缺失、不可读、不完整或计数自相矛盾；显示中性“指标来源尚未核验”，核心检测仍可用。此时 `/api/metrics*` 也会返回 404 或 503，不要出现健康状态说未核验、指标页却继续展示正式分数的矛盾状态。

状态区只展示简短结论，详细原因放在指标来源抽屉。该状态不是模型加载失败，不能据此禁用上传。

### 15. 生产部署的 API 地址与并发状态

现状：`src/lib/api.ts` 使用相对 `/api`，配合 Vite 开发代理是正确的；但构建后的静态文件若不与后端同源部署，也没有反向代理，就会请求错误的主机。

建议：
- 默认继续使用相对 `/api`，生产环境优先由 Web 服务器反向代理，保持同源。
- 如确实需要前后端分域，增加可选的 `VITE_API_BASE_URL`，在一个 API helper 中统一去除末尾斜杠并拼接路径，不要在各页面散落主机地址。
- 分域部署时在后端设置 `MALGUARD_CORS_ORIGINS` 为前端的完整来源；不要让前端发送 `Access-Control-Allow-Origin`，该响应头只能由服务端设置。
- `GET /api/health` 的 `inferenceConcurrency` 可放入诊断详情，不需要作为首页功能卖点。前端不能据此伪造队列进度，也不要并发拆分一个批量请求。

验收：相对代理模式和 `VITE_API_BASE_URL` 模式各完成一次 health、单文件检测和历史读取；未配置来源时明确显示网络/跨域错误，不回退到 mock。

### 16. 可选 API Key 接入

后端设置 `MALGUARD_API_KEY` 后，检测与全部历史接口要求 `X-API-Key`，但 health、ready 和 metrics 仍公开。

建议：
- 启动时读取 `GET /api/health` 的 `apiKeyRequired`。值为 `true` 且当前会话没有密钥时，禁用上传与历史请求，并提供一个密码类型的运行时输入框。
- 密钥只保存在内存或 `sessionStorage`，不要使用 `VITE_*` 构建变量、源码常量、URL 查询参数、`localStorage` 或日志；Vite 构建变量会被打包进公开 JavaScript，不能保存秘密。
- 在统一 API helper 中仅对受保护请求添加 `X-API-Key`，并集中处理 401。401 时清除当前会话凭据并要求重新输入，不能回退到 mock。
- 历史 HTML 报告不能继续使用普通 `<a href>`：用带密钥的 `fetch` 获取 HTML，验证响应成功后创建 Blob URL 并打开，随后及时 `URL.revokeObjectURL()`。
- 分域部署时仍使用 HTTPS；API Key 不能替代 TLS，也不能阻止已经能操作浏览器页面的用户查看会话内密钥。

验收：分别在未配置密钥、正确密钥、错误密钥和密钥过期/变更四种状态下验证单文件、批量、历史列表、删除及报告打开；开发模式 CORS 预检不得因缺少密钥失败。

### 17. 历史数据库备份

增加一个下载图标按钮调用 `GET /api/history/backup`。该接口返回可独立打开的 SQLite 附件，不是 JSON。

建议：
- 使用统一 API helper 发起请求，以便鉴权模式自动携带 `X-API-Key`；不能使用会绕过请求头处理的普通下载链接。
- 成功后读取 Blob，优先使用响应 `Content-Disposition` 中的安全文件名，缺失时回退到 `malguard-history.db`；创建临时下载 URL 后立即撤销。
- 下载期间禁用按钮并保持布局尺寸稳定；401、500 和网络错误使用现有统一错误机制及请求编号。
- “清空历史”确认框可提供“先下载备份”的独立操作，但备份失败时不得自动继续清空。
- 不增加在线恢复或数据库上传按钮。恢复需要停服处理数据库和 WAL/SHM 文件，应由运维人员完成。

验收：有记录、空数据库、API Key 开关两种模式下均能下载并用 SQLite 打开；连续下载后浏览器不残留 Blob URL，服务端目录不残留 `.history-backup-*.db`。

### 18. 检测繁忙与重试状态

后端达到 `MALGUARD_DETECTION_CONCURRENCY` 时，单文件和批量检测会在读取上传前返回 429，并带 `Retry-After: 1`。该响应不会创建历史记录。

建议：
- API 层增加独立的“服务繁忙”错误类型，保留用户已选择的文件，不要清空批量列表或生成失败历史项。
- 读取 `Retry-After` 显示短倒计时和明确的重试按钮；最多自动重试一次，不能无限循环或在多个组件中各自重试。
- 本页面已有检测请求时禁用重复提交，但不能把本地按钮状态当作全局容量判断；其他标签页和客户端仍可能占用槽位。
- `GET /api/health` 的 `detectionConcurrency` 只放在诊断详情，不显示虚构的实时剩余槽位。后端目前没有公开 active count 或队列位置。
- 429 与 413 不同：429 可在稍后原样重试，413 必须减少文件数或体积。

验收：将后端并发上限设为 1，同时从两个浏览器标签提交，第二个请求显示繁忙状态且文件仍可重试；第一个完成后重试成功，历史中只出现成功执行的记录。
